...
fn it_works() {
    ...
    let pool = my::Pool::new(format!(
        "mysql://root:{}@db:3306",
        env::var("MYSQL_ROOT_PASSWORD").unwrap()
    )).unwrap();
    ...
    */
    ...
    pool.prep_exec(r"CREATE DATABASE tmp", ()).unwrap();

    pool.prep_exec(
        r"CREATE TABLE tmp.payment (
                     customer_id int not null,
                     amount int not null,
                     account_name text
                 )",
    ...
    pool.prep_exec(r"DROP TABLE tmp.payment", ()).unwrap();

    pool.prep_exec(r"DROP DATABASE tmp", ()).unwrap();
